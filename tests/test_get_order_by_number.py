...
class TestGetOrderByNumber:
    ...
    def test_get_order_by_number_success(self, order_client, order_id):
        response = order_client.get_order_by_number(order_id)

        assert response.status_code == HTTPStatusCodes.CODE_200_OK['status_code']
        assert response.json()['order']['track'] == order_id
